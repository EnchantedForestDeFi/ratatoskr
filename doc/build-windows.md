...
Below are some notes on how to build Ratatoskr Core for Windows.

The options known to work for building Ratatoskr Core on Windows are:
...
Note that for WSL the Ratatoskr Core source path MUST be somewhere in the default mount file system, for
example /usr/src/ratatoskr, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\ratatoskr`, for example:

    make install DESTDIR=/mnt/c/workspace/ratatoskr
