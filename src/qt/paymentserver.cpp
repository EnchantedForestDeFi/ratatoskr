...
const QString BITCOIN_IPC_PREFIX("ratatoskr:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // ratatoskr: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click ratatoskr: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start ratatoskr: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling ratatoskr: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("ratatoskr://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'ratatoskr://' is not a valid URI. Use 'ratatoskr:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // ratatoskr: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Ratatoskr address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
