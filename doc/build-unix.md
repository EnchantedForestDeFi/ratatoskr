UNIX BUILD NOTES
====================
Some notes on how to build Ratatoskr Core in Unix.
...
memory available when compiling Ratatoskr Core. On systems with less, gcc can be
...
To build Ratatoskr Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
If you want to build ratatoskr-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a ratatoskr-qt executable will be
built by default.
...
Note: Fedora repositories do not include libbacktrace. To build Ratatoskr Core without stack trace support, configure with `--disable-stacktraces`.
...
...
to: /path/to/ratatoskr/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/ratatoskr/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Ratatoskr Core can
