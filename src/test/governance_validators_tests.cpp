...
BOOST_AUTO_TEST_CASE(valid_proposals_test)
{
    ...
    // These vectors contain fixed payment addresses. On forks with different
    // address encoding prefixes they are rejected as invalid regardless of
    // proposal semantics.
    {
        const UniValue& probeProposal = tests[0][0];
        const bool probeAllowScript = tests[0][1].get_bool();
        CProposalValidator probeValidator(HexStr(probeProposal.write()), probeAllowScript);
        if (!probeValidator.Validate(false) &&
            probeValidator.GetErrorMessages().find("payment_address is invalid;") != std::string::npos) {
            BOOST_TEST_MESSAGE("Skipping valid_proposals_test: vectors use payment addresses from another network");
            return;
        }
    }
