...
Since version 0.21, Ratatoskr Core no longer has a default wallet.
...
The following command, for example, creates a descriptor wallet. More information about this command may be found by running `ratatoskr-cli help createwallet`.

```
$ ratatoskr-cli -named createwallet wallet_name="wallet-01" descriptors=true
```
...
| -----------------|:------------------------------------------------------------|
| Linux            | `/home/<user>/.ratatoskrcore/wallets`                      |
| Windows          | `C:\Users\<user>\AppData\Roaming\RatatoskrCore\wallets`    |
| macOS            | `/Users/<user>/Library/Application Support/RatatoskrCore/wallets` |
...
```
$ ratatoskr-cli -rpcwallet="wallet-01" encryptwallet "passphrase"
```

Once encrypted, the passphrase can be changed with the `walletpassphrasechange` command.

```
$ ratatoskr-cli -rpcwallet="wallet-01" walletpassphrasechange "oldpassphrase" "newpassphrase"
```
...
```
$ ratatoskr-cli -named createwallet wallet_name="wallet-01" descriptors=true passphrase="passphrase"
```
...
```
$ ratatoskr-cli -rpcwallet="wallet-01" sendtoaddress "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx" 0.01
error code: -13
error message:
...
```
$ ratatoskr-cli -rpcwallet="wallet-01" walletpassphrase "passphrase" 120
```
...
```
$ ratatoskr-cli -rpcwallet="wallet-01" backupwallet /home/node01/Backups/backup-01.dat
```
...
```
$ ratatoskr-cli restorewallet "restored-wallet" /home/node01/Backups/backup-01.dat
```

After that, `getwalletinfo` can be used to check if the wallet has been fully restored.

```
$ ratatoskr-cli -rpcwallet="restored-wallet" getwalletinfo
```
