// Copyright (c) 2025 The Ratatoskr Core developers
...
static uint16_t MainnetP2PPort()
{
    return CreateChainParams(gArgs, CBaseChainParams::MAIN)->GetDefaultPort();
}

static uint16_t AlternateP2PPort(const uint16_t main_port)
{
    return main_port == 9999 ? 9998 : 9999;
}

static std::string AddrPort(const std::string& host, const uint16_t port)
{
    return strprintf("%s:%u", host, port);
}

static std::vector<TestEntry> GetAddrValsMain()
{
    const uint16_t main_port = Params().GetDefaultPort();
    const uint16_t alt_port = AlternateP2PPort(main_port);

    return {
    // Address and port specified
    {{NetInfoPurpose::CORE_P2P, AddrPort("1.1.1.1", main_port)}, NetInfoStatus::Success, NetInfoStatus::Success},
    ...
    // - ExtNetInfo is indifferent to choice of port unless it's a bad port
    {{NetInfoPurpose::CORE_P2P, AddrPort("1.1.1.1", alt_port)}, NetInfoStatus::BadPort, NetInfoStatus::Success},
    // Internal addresses not allowed on mainnet
    {{NetInfoPurpose::CORE_P2P, AddrPort("127.0.0.1", main_port)}, NetInfoStatus::NotRoutable, NetInfoStatus::NotRoutable},
    ...
    {{NetInfoPurpose::CORE_P2P, AddrPort("pg6mmjiyjmcrsslvykfwnntlaru7p5svn6y2ymmju6nubxndf4pscryd.onion", main_port)}, NetInfoStatus::BadInput, NetInfoStatus::Success},
    {{NetInfoPurpose::PLATFORM_P2P, AddrPort("example.com", main_port)}, NetInfoStatus::MaxLimit, NetInfoStatus::BadInput},
    {{NetInfoPurpose::PLATFORM_P2P, AddrPort("pg6mmjiyjmcrsslvykfwnntlaru7p5svn6y2ymmju6nubxndf4pscryd.onion", main_port)}, NetInfoStatus::MaxLimit, NetInfoStatus::Success},
    // - MnNetInfo doesn't allow storing anything except a Core P2P address
    // - ExtNetInfo can store Platform HTTPS addresses *as domains* alongside privacy network domains
    {{NetInfoPurpose::PLATFORM_HTTPS, AddrPort("example.com", main_port)}, NetInfoStatus::MaxLimit, NetInfoStatus::Success},
    {{NetInfoPurpose::PLATFORM_HTTPS, AddrPort("pg6mmjiyjmcrsslvykfwnntlaru7p5svn6y2ymmju6nubxndf4pscryd.onion", main_port)}, NetInfoStatus::MaxLimit, NetInfoStatus::Success},
    // Incorrect IPv4 address
    {{NetInfoPurpose::CORE_P2P, AddrPort("1.1.1.256", main_port)}, NetInfoStatus::BadInput, NetInfoStatus::BadInput},
    // Missing address
    {{NetInfoPurpose::CORE_P2P, strprintf(":%u", main_port)}, NetInfoStatus::BadInput, NetInfoStatus::BadInput},
    // Bad purpose code
    {{static_cast<NetInfoPurpose>(64), AddrPort("1.1.1.1", main_port)}, NetInfoStatus::MaxLimit, NetInfoStatus::MaxLimit},
    // - MnNetInfo doesn't allow storing anything except a Core P2P address
    // - ExtNetInfo allows storing Platform P2P addresses
    {{NetInfoPurpose::PLATFORM_P2P, AddrPort("1.1.1.1", main_port)}, NetInfoStatus::MaxLimit, NetInfoStatus::Success},
    };
}
...
{
    const auto main_vals = GetAddrValsMain();
    const uint16_t main_port = Params().GetDefaultPort();
    TestMnNetInfo(main_vals);

    {
        // MnNetInfo only stores one value, overwriting prohibited
        MnNetInfo netInfo;
        BOOST_CHECK_EQUAL(netInfo.AddEntry(NetInfoPurpose::CORE_P2P, AddrPort("1.1.1.1", main_port)), NetInfoStatus::Success);
        BOOST_CHECK_EQUAL(netInfo.AddEntry(NetInfoPurpose::CORE_P2P, AddrPort("1.1.1.2", main_port)), NetInfoStatus::MaxLimit);
...

    {
        ...
        for (const auto purpose : {NetInfoPurpose::PLATFORM_HTTPS, NetInfoPurpose::PLATFORM_P2P}) {
            BOOST_CHECK_EQUAL(netInfo.AddEntry(purpose, AddrPort("1.1.1.1", main_port)), NetInfoStatus::MaxLimit);
            BOOST_CHECK(!netInfo.HasEntries(purpose));
        }
        BOOST_CHECK(netInfo.GetEntries().empty());
    }
...
BOOST_AUTO_TEST_CASE(extnetinfo_rules_main) { TestExtNetInfo(GetAddrValsMain()); }

static std::vector<TestEntry> GetAddrValsReg()
{
    const uint16_t main_port = MainnetP2PPort();
    ...
    // - ExtNetInfo is indifferent to choice of port unless it's a bad port
    {{NetInfoPurpose::CORE_P2P, AddrPort("1.1.1.1", main_port)}, NetInfoStatus::BadPort, NetInfoStatus::Success},
...
BOOST_FIXTURE_TEST_CASE(mnnetinfo_rules_reg, RegTestingSetup) { TestMnNetInfo(GetAddrValsReg()); }

BOOST_FIXTURE_TEST_CASE(extnetinfo_rules_reg, RegTestingSetup)
{
    TestExtNetInfo(GetAddrValsReg());
...
BOOST_AUTO_TEST_CASE(cservice_compatible)
{
    ...
    service = LookupNumeric("1.1.1.1", main_port);
    netInfo.Clear();
    BOOST_CHECK_EQUAL(netInfo.AddEntry(NetInfoPurpose::CORE_P2P, AddrPort("1.1.1.1", main_port)), NetInfoStatus::Success);
    BOOST_CHECK(CheckIfSerSame(service, netInfo));
...
BOOST_AUTO_TEST_CASE(interface_equality)
{
    ...
    BOOST_CHECK_EQUAL(ptr_rhs->AddEntry(NetInfoPurpose::CORE_P2P, AddrPort("1.1.1.1", main_port)), NetInfoStatus::Success);
    BOOST_CHECK(!util::shared_ptr_equal(ptr_lhs, ptr_rhs) && util::shared_ptr_not_equal(ptr_lhs, ptr_rhs));
