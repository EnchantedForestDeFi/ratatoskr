...
void setApplicationFont()
{
    ...
    // Keep native system font size to avoid oversized UI on high DPI setups.
    if (family != OS_FONT_STR) {
...
{
    FontAttrib normalized{font_attrib};
    if (normalized.m_point_size > 0) {
        // Clamp custom point sizes to keep UI proportions close to Bitcoin Core.
        normalized.m_point_size = std::min(normalized.m_point_size, 11.0);
    }
    for (auto it : vecWidgets) {
        auto itFontUpdate = mapFontUpdates.emplace(std::make_pair(it, normalized));
        if (!itFontUpdate.second) {
            itFontUpdate.first->second = normalized;
        }
    }
}
