# User-space, Statically Defined Tracing (USDT) for Ratatoskr Core

Ratatoskr Core includes statically defined tracepoints to allow for more
...
**Note**: Ratatoskr Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Ratatoskr Core
...
Multiple tools can list the available tracepoints in a `ratatoskrd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Ratatoskr Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/ratatoskrd
stap net        outbound_message 0x0000000000107c05 /src/ratatoskrd
stap validation block_connected  0x00000000002fb10c /src/ratatoskrd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Ratatoskr Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/ratatoskrd | grep NT_STAPSDT -A 4 -B 2
