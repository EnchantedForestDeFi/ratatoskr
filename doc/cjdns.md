# CJDNS support in Ratatoskr Core

It is possible to run Ratatoskr Core over CJDNS, an encrypted IPv6 network that
...
done outside of Ratatoskr Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Ratatoskr network and individual nodes.
...
before it will work with your Ratatoskr Core node. This is described in steps
...
## Run Ratatoskr Core with CJDNS

Once you are connected to the CJDNS network, the following Ratatoskr Core
...
When enabled, this option tells Ratatoskr Core that it is running in an
...
IPv6 local network. This helps Ratatoskr Core perform better address management:
...
CJDNS support was added to Ratatoskr Core in version 21.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `ratatoskr-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/SmartiesCoin/Smartiecoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Ratatoskr Core

There are several ways to see your CJDNS address in Ratatoskr Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `ratatoskr-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `ratatoskr-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `ratatoskr-cli help getnodeaddresses` for details.
