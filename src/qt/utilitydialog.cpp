...
For more information, see the <a style=\"%2\" href=\"%3\">%1 documentation</a>."
        ...
        QString miningHtml =
            ...
            + "<p>" + tr("Example, pointing cpuminer-opt at the EnchantedForestDeFi PPLNS pool:") + "</p>"
            + "<pre style=\"background:rgba(127,127,127,0.12);padding:6px;border-radius:4px\">"
              ...
              "  -u &lt;your_RATR_address&gt; -p x"
              "</pre>"
            + "<p>" + tr("Pool endpoints:") + "</p>"
            + "<ul>"
            + "<li><b>" + tr("EU (primary)") + ":</b> pool.ratatoskr.enchantedforestdefi.com — "
                         "3035 (PPLNS, 1.5% fee), 3036 (SOLO, 2.0% fee)</li>"
            + "<li><b>" + tr("EU (alt engine)") + ":</b> pool2.ratatoskr.enchantedforestdefi.com — same ports</li>"
            + "<li><b>" + tr("US") + ":</b> pool-us.ratatoskr.enchantedforestdefi.com — same ports</li>"
            + "</ul>"
            + "<p>" + tr("Pool dashboard") + ": "
            + "<a style=\"" + linkStyle + "\" href=\"https://pool.ratatoskr.enchantedforestdefi.com\">"
            + "pool.ratatoskr.enchantedforestdefi.com</a></p>"
            + "<p>" + tr("Other pools (e.g. rplant.xyz) also support RATR — same algorithm + miner, "
                         "different stratum URL.") + "</p>"
        ...
        QString welcomeHtml =
            ...
            + "<li>" + tr("Manage masternodes from the <i>Masternodes</i> "
                         ...
                         "remote node. <b>Tip:</b> after receiving the 7,500 RATR "
                         "collateral, enable coin control (<i>Settings → Options → "
                         "Wallet → Enable coin control features</i>) and right-click "
                         "the UTXO → <i>Lock Unspent</i> so it can't be accidentally spent.") + "</li>"
            + "<li>" + tr("Prefer a different look? <i>Settings → Options → Appearance</i> "
                         "lets you switch themes — try <b>Parchment</b> or <b>Forest</b> "
                         "for the project's brand aesthetic.") + "</li>"
            + "</ul>"
